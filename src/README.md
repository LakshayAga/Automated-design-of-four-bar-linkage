...
| `generate_and_save_validation.py` | Generates the preliminary dataset (500 samples with raw trajectories + FDs), trains the preliminary model, and persists both to `data/` and `models/`. |
...
- **`models/preliminary_model.pth`** — Trained preliminary model state dict.
- **`models/preliminary_training_meta.pt`** — Loss history, hyperparameters, architecture metadata.

## Current State
- The preliminary pipeline is functional and persists model weights and training data to disk.
